...
export class TaskTypeComponent {
 ...
 // Show modal const el = document.querySelector("#programmatically-modal"); const modal = tailwind.Modal.getOrCreateInstance(el); modal.show(); // Hide modal const el = document.querySelector("#programmatically-modal"); const modal = tailwind.Modal.getOrCreateInstance(el); modal.hide(); // Toggle modal const el = document.querySelector("#programmatically-modal"); const modal = tailwind.Modal.getOrCreateInstance(el); modal.toggle(); 
  ...
  getList(){
    let url ="user/task_types?company_id=" + this.company_id + "&page=" +  this.page.page +"&perPage=" +this.page.size +"&keyword=" +this.keyword;
  ...
  confirmAction(event:any){
    ...
    if(event['type']=="delete"){
      this.apiService.delete("user/deletetask_types?id="+event['id']).subscribe((res) => {
        if (res["status"] == true) {
          this.eventService.broadcast('success', {page:'Company',message:'Deleted selected task.'});
        this.getList()
        }
      });
    }else{
      this.apiService.put(`user/statustask_types?value=${event['type']=='enable'?1:0}&id=${event['id']}`,{}).subscribe((res) => {
        if (res["status"] == true) {
          ...
          this.eventService.broadcast('success', {page:'Company',message:'Selected task has been enabled.'});
          else
          this.eventService.broadcast('success', {page:'Company',message:'Selected task has been disabled.'});
         this.getList()
        }
      });
    }
